...
const LabourProfile = () => {
  const [name, setName] = useState("Labour Contractor X");
  const [address, setAddress] = useState("789 Pine Street, City, State");
  const [aadhar, setAadhar] = useState("9876 5432 1098");
  const [gstNumber, setGstNumber] = useState(""); // optional
  const [phone, setPhone] = useState("9876543210");

  const saveProfile = () => {
    if (!name || !address || !aadhar || !phone) {
    ...
    if (!/^\d{4}\s\d{4}\s\d{4}$/.test(aadhar)) {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.header}>Labour Contractor Profile</Text>
